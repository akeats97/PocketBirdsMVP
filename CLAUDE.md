...
## iOS / TestFlight Setup — in progress (May 22 2026)

Goal: ship PocketBirds to friends' iPhones via TestFlight. Bundle ID `com.akeats97.pocketbirds` (same as Android). Plan file: `~/.claude/plans/i-want-my-friends-humming-lighthouse.md`.

### Status (May 22 2026, ~3:50 PM EDT)

**Apple Developer Program:** enrolled May 22 2026, approved same day.

**App registered:**
- Apple Developer → Identifiers → App ID `com.akeats97.pocketbirds` with Push Notifications capability.
- App Store Connect → My Apps → PocketBirds (SKU `pocketbirds-ios`).
- Agreements, Tax, and Banking: Free Apps Agreement (verify accepted on resume).

**Credentials created:**
- **APNs Auth Key** (for Firebase Cloud Messaging → APNs)
  - Key ID: `6TBT96JT76`
  - Team ID: `WB8PNB2CCR`
  - .p8 file: `~/Downloads/AuthKey_6TBT96JT76.p8` — **move to 1Password before session ends**.
  - Uploaded to Firebase: <https://console.firebase.google.com/project/pocketbirds/settings/cloudmessaging> both dev and prod slots populated with the same key.
- **App Store Connect API Key** (for `eas submit -p ios`)
  - Key ID: `GFYGV5D864`
  - Issuer ID: `877b4389-81c0-4cd8-8f5e-7e7a7f506e19`
  - Role: App Manager
  - .p8 file: `~/Downloads/AuthKey_GFYGV5D864.p8` — **move to 1Password before session ends**.
- **EAS-managed iOS credentials** (auto-generated during first build, May 22 2026)
  - Apple Distribution Certificate: serial `192C24093D549D4CDA4BCF6FCDF0F32F`, expires 2027-05-22.
  - Provisioning Profile: Apple ID `V4H2K892QC`, expires 2027-05-22.
  - Second APNs key was auto-generated by EAS (uses second of 2 team slots). Independent from the Firebase key.
...
- `eas.json`: empty `submit.production.ios` block so EAS prompts for ASC API key on first submit.
- `ios/PocketBirds4/GoogleService-Info.plist` committed. Note: not yet wired into the Xcode project (no PBXFileReference). Push works without this since Expo → FCM → APNs flow does not need the native Firebase iOS SDK. Only relevant if we add Firebase Analytics or similar later.
- App icon + splash redone in cream `#fdf6e5` with bold black "PB" wordmark (icon.png, adaptive-icon.png, splash-icon.png).

**Privacy / Deletion URLs (live, served by GitHub Pages on `akeats97/PocketBirdsMVP`):**
- <https://akeats97.github.io/PocketBirdsMVP/PRIVACY>
- <https://akeats97.github.io/PocketBirdsMVP/DELETION>

**Build status:** First iOS production build queued at EAS at ~3:50 PM EDT May 22 (~20-30 min to complete). buildNumber=2, autoincrement on. Find at <https://expo.dev/accounts/akeats97/projects/PocketBirds4/builds>.

**Capability sync warning to verify:** During credential setup, EAS logged `Synced capabilities: Disabled: Push Notifications` — ambiguous phrasing. After build, verify `ios/PocketBirds4/PocketBirds4.entitlements` includes `aps-environment` key. If missing, push won't work on iOS even though Firebase is wired up. Add the key and rebuild.

### Next steps (in order)

1. **Wait for build to finish.** Check <https://expo.dev/accounts/akeats97/projects/PocketBirds4/builds>.
2. **Verify the entitlements file** has `aps-environment` set to `development` (or use Expo's entitlements modification mechanism if missing).
3. **Submit to App Store Connect:** `eas submit --platform ios --latest`. EAS will prompt for ASC API key on first run — point at `~/Downloads/AuthKey_GFYGV5D864.p8`, Key ID `GFYGV5D864`, Issuer ID `877b4389-81c0-4cd8-8f5e-7e7a7f506e19`.
4. **Wait for Apple processing** (~10-30 min after upload). Build appears in App Store Connect → PocketBirds → TestFlight as "Ready to Test".
5. **Fill in TestFlight beta info:** Beta App Description, Feedback Email, Test Information (what to focus on). Privacy URL already set when registering the app — re-use the GitHub Pages link.
6. **Create External Group "Friends"** → add testers by email → submit for Beta App Review (~24-48h on first build, instant on subsequent unless major changes).
7. **Friends install TestFlight from App Store → accept invite email → install PocketBirds.**

### Architecture refresher (so future-Claude doesn't re-derive)

- We use Firebase JS SDK only (not @react-native-firebase). The iOS plist is committed for forward compatibility, not for any active runtime use.
