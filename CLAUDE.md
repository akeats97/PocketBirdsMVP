...
- **Victoria's location autofill is broken.** On Victoria's phone the location field doesn't suggest places as she types, and the locate-me crosshair may also not be firing. Suggests the Google Places API isn't being called for her at all (or is being called and failing silently). Things to check: (a) is the `EXPO_PUBLIC_GOOGLE_PLACES_API_KEY` env var present in the production build she's running, (b) is her Android package + SHA-1 covered by the API key's app restrictions in GCP (she's on the Play Store build, signed with the new keystore — different SHA-1 from the old one), (c) network/permission errors in `placesService.ts` being swallowed. Repro on a fresh Play Store install, then check GCP API metrics to see if any requests are landing.
