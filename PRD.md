...
6. **Photos are celebrated, and now strongly encouraged.** _(REVISED Jul 1 2026 — the old "never required" stance softened, Alex's call.)_ As the user base grew past the original two-person graph, a photoless sighting started to feel thin: nobody wants to scroll a feed of pictureless entries. So the Add flow now leads with the photo and makes it the default, encouraged path (photo-first, with the skip de-emphasized to a quiet link). We still do **not hard-require** a photo, because that would break Mystery Bird, heard-only birds, and quick/backlog logging where a photo is impossible. The principle is now "strongly suggest, never wall off." A photoless sighting is still a valid sighting; it just isn't the path we lead people down. Features are still never gated behind having a photo. _(Original text, for history: "Photos are celebrated, never required. A sighting without a photo is a first-class sighting... the app never demands one and never gates features behind one.")_
