...
>
> **Recovery layer (Jun 26 2026).** AVONET/IUCN misses are mostly IOC genus-renames/splits filed under a synonym, not absent data. `build-species-data.py` recovers them via (a) **genus-synonym** — same species epithet + the new genus is one IOC uses in the *same family* (the family guard rejects coincidental cross-family epithet collisions, e.g. Red Grouse must not grab the crossbill *Loxia scotica*), and (b) **AVONET crosswalk** (BirdLife↔eBird/BirdTree). Recovered +71 AVONET, +118 IUCN. **Every recovered row is logged in `scripts/species-data-recovered.csv`** (dataset, IOC name/Latin, method, confidence, matched source name, value, "corroborated by IUCN" note) for **Alex to manually review** — 189 rows; the 14 `low`-confidence crosswalk split/lumps (extinct island birds assigned a close relative's traits) are the top review priority.
>
